...
psi_int_lt_m(:,2)=phi_int_lt;
psi_ext_lt_m(:,2)=phi_ext_lt;
tP_mx=zeros(M,16);
dth=2*pi/(n_th-1);
theta=0:dth:2*pi;
...
data_int_lt=psi_int_lt_m*(tP_mx);
data_ext_lt=psi_ext_lt_m*(tP_mx);
...
[C1,h1]=contour(x_int,y_int,data_int,[-0.2,0.2,-0.6,0.6,1.0,-1.0],'b');
[C3,h3]=contour(x_int,y_int,data_int_lt,[-0.2,0.2,-0.6,0.6,1.0,-1.0],'r--');
[C2,h2]=contour(x_ext,y_ext,data_ext,[-0.3,0.3,-0.7,0.7,1.1,-1.1,-1.5,1.5]*2,'b');
[C4,h4]=contour(x_ext,y_ext,data_ext_lt,[-0.3,0.3,-0.7,0.7,1.1,-1.1,-1.5,1.5]*2,'r--');
...
jpg_name=strcat('fig_t_phi_',num2str(cur_time_i),'.fig');
saveas(gcf,jpg_name);
hold off
drawnow
